...
table1(~ country + Q_Language + academic_status + gender + age_cat + polinterest_cat_3 + empathy_pc_cat + exp_hate_speech_cat + exp_hostile_engagement_cat + commitment_cat, data = data)
```
...
combined_summary_missing %>%
  ...
  scale_x_discrete(na.translate = FALSE) +
  scale_y_continuous(limits = c(0, 0.4)) + # Set uniform Y-axis limits
  coord_flip() +
  theme_minimal() +
