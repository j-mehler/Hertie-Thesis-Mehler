---
title: Batches Correlation Analysis"
...
report_columns_with_only_zeros <- function(data) {
  ...
  data_cleaned <- data %>% select(-all_of(columns_with_zeros))
  
  if(length(columns_with_zeros) > 0) {
    cat("Columns with only zeros that were removed:", paste(columns_with_zeros, collapse = ", "), "\n")
  } else {
...
report_columns_with_only_zeros(data_content)
report_columns_with_only_zeros(data_other_features)
report_columns_with_only_zeros(data_target)
report_columns_with_only_zeros(data_sender)
report_columns_with_only_zeros(data_scope)
```
...
```{r}
# dataframe containing only boolean variables
cor_matrix <- cor(data_numeric_filtered, method = "pearson")

# Create a PNG device using Cairo
#CairoPNG(filename = here("figures", "2-04-item_correlations.png"))
...
# save figure
ggsave("2-04-corr-content.png", path = here("figures"))
```
...
cor_data <- as.data.frame(as.table(cor_matrix)) %>%
  ...
  filter(abs(Freq) > 0.3) # Filter to simplify the network, adjust threshold as needed
