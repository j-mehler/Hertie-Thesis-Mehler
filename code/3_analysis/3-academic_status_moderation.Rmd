...
# change value names that are too long to be displayed without line breaks
data <- data %>% mutate(leftright_cat = recode(leftright_cat, 
                          "liberal/left" = "liberal/\nleft", 
                          "moderate/center" = "moderate/\ncenter", 
                          "conservative/right" = "conservative/\nright"),
                        exp_hate_speech_cat = recode(exp_hate_speech_cat,
                          "Less experience" = "Less\nexperience", 
                          "More experience" = "More\nexperience"),
                        exp_hostile_engagement_cat = recode(exp_hostile_engagement_cat,
                          "Less experience" = "Less\nexperience", 
                          "More experience" = "More\nexperience")
                        )
...
multinom_model_interaction <- function(data, var1, var2) {
  ...
  # Use plot_model() to visualize the interaction
  plot <- plot_model(MODEL, type = "eff", 
                     terms = c(var1, "academic_status"), 
                     title = paste("Predicted probabilities of", var2, "\nby", var1, "moderated by Academic Status", "\ncontrolled for Gender, Age and Survey Commitment"), 
                     colors = c("skyblue", "orange"), 
                     axis.title = c(var1, "Effect"), 
                     legend.title = "Academic Status",
                     ci.lvl = 0.95) +
    theme_bw() +
    coord_flip()
